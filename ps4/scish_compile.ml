...
let binop_of_prim p : Cish_ast.binop =
  match p with
  | Scish_ast.Plus -> Plus
...
let lookup (en: environment) (v: var) : Cish_ast.exp =
  let rec helper en v acc =
    match en with
    | [] -> raise VariableNotFoundInEnvironment
...
and compile_primapp (op: Scish_ast.primop) (es: Scish_ast.exp list) (en: environment) : Cish_ast.stmt =
  match op with
